        ...
        <>
            ...
            <div className="space-y-6">
                ...
                >
                    ...
                    <motion.div className="md:col-span-2" variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }}>
                ...
                >
                    <Card className="xl:col-span-2 rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
                        <CardHeader>
                            ...
                            {chart.length === 0 ? (
                                ...
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={chart} margin={{ top: 8, right: 8, left: 0, bottom: 8 }}>
                                        ...
                                        />
                                        <Bar dataKey="estimasi" fill="var(--chart-1)" radius={[8, 8, 0, 0]} animationDuration={1500} />
                                    </BarChart>
                                </ResponsiveContainer>
                            )}
                        </CardContent>
                    ...
                    <Card className="rounded-2xl border border-border bg-card shadow-sm">
                        ...
                        <CardContent className="space-y-2">
                            <Button asChild className="w-full justify-start transition-all hover:translate-x-1">
                                <Link href="/projects/create">
                                    <FolderPlus className="h-4 w-4 mr-2" /> Tambah Proyek
                                </Link>
                            </Button>
                            <Button asChild variant="secondary" className="w-full justify-start transition-all hover:translate-x-1">
                                <Link href="/inputs/create">
                                    <ClipboardPlus className="h-4 w-4 mr-2" /> Input Data
                                </Link>
                            </Button>
                            <Button asChild variant="accent" className="w-full justify-start transition-all hover:translate-x-1">
                                <Link href="/histories">
                                    <Sparkles className="h-4 w-4 mr-2" /> Lihat Riwayat
                                </Link>
                            </Button>
                            <Button asChild variant="outline" className="w-full justify-start transition-all hover:translate-x-1">
                                <Link href={stats.latest_prediction ? '/histories' : '/inputs/create'}>
                                    <Sparkles className="h-4 w-4 mr-2" /> Lihat Rekomendasi
                                </Link>
                            </Button>
                            <Button asChild variant="outline" className="w-full justify-start transition-all hover:translate-x-1">
                    ...
                    <Card className="xl:col-span-3 rounded-2xl border border-border bg-card shadow-sm">
                        ...
                        <CardContent className="space-y-3">
                            {recentHistories.length === 0 ? (
                                ...
                                recentHistories.map((history, idx) => (
                                    <motion.div
                                        key={history.id}
                                        className="flex items-center justify-between rounded-2xl border border-border px-4 py-3 hover:bg-muted/30 transition-colors"
